...
ZIPNAME="GenerKernel-AOSP-dumpling-$(TZ=Asia/Shanghai date +"%Y%m%d-%H%M").zip"
#ZIPNAME_KSU="RyzenKernel-AOSP-Ginkgo-KSU-$(TZ=Asia/Shanghai date +"%Y%m%d-%H%M").zip"
...
export KBUILD_BUILD_USER="douniwan"
...
elif ! git clone -q https://github.com/linlinger/AnyKernel3; then
...
rm -f *zip
cd AnyKernel3
git checkout dumpling &> /dev/null
if [[ $1 = "-k" || $1 = "--ksu" ]]; then
